...
class Primeproduction_model extends CI_Model {
    ...
    public function getDataProductionMcpByDate(string $date, int $typeCritere = null) : array
    {
        $this->db->select('SUM(mcp_quantite) AS nombre_prod', false)
            ->select('usr_id, usr_nom, usr_prenom')
            ->select('campagne_id, campagne_libelle')
            ...
            ->order_by('usr_id ASC')
            ;
        if($typeCritere != '') $this->db->where('primecritere_typecritere', $typeCritere);
    ...
    {
        $this->db
            ->select('prime_id, prime_basemensuelle, prime_totalbonus as bonus, prime_totalmalus as malus, prime_montantpercu, prime_montantqualite, prime_datecrea, prime_datemodif')
            ->select('campagne_libelle')
    ...
    public function updatePrimeByUserAndCampagne(int $user, int $campagne, array $data)
    {
        $this->db->where('prime_user', $user);
        $this->db->where('prime_campagne', $campagne);
        $this->db->update('t_prime', $data);

        if($this->db->affected_rows()) return true;
        return false;
    }

    public function importQualite(array $datas) : bool
    {
        foreach($datas as $data)
        {
            ...
            $prime_mois = $data['mois'];
            $prime_annee = $data['annee'];
            $type = $data['type'];
            $commentaire = $data['commentaire'];
    
            $this->db
                    ->where('primequalite_user', $user)
                    ->where('primequalite_campagne', $campagne)
                    ->where('primequalite_mois', $prime_mois)
                    ->where('primequalite_annee', $prime_annee)
                    ->where('primequalite_typecritere', $type)
                    ;
            $this->db->update('t_primequalite', ['primequalite_note' => $note, 'primequalite_datemodif' => date('Y-m-d H:i:s')]);
        }
        if(!$this->db->affected_rows()){
            $insertData = [
                'primequalite_user' => $user,
                'primequalite_campagne' => $campagne,
                'primequalite_typecritere' => $type,
                'primequalite_mois' => $prime_mois,
                'primequalite_annee' => $prime_annee,
                'primequalite_note' => $note,
                'primequalite_commentaire' => $commentaire,
                'primequalite_datecrea' => date('Y-m-d H:i:s'),
                'primequalite_datemodif' => date('Y-m-d H:i:s')
            ];
            $this->db->insert('t_primequalite', $insertData);
        }
        if(!$this->db->affected_rows()) return false; 
        return true;
    }

    public function getPrimeAjustementByMonthAndYear(int $month, int $year)
    {
        $this->db
                    ->select('SUM(primeajustement_bonus) as bonus, SUM(primeajustement_malus) as malus')
                    ->select('prime_campagne, prime_user')
                    ->join('t_prime', 'primeajustement_prime = prime_id')
                    ->where('primeajustement_datevalidation IS NOT NULL', null, false)
                    ->where('primeajustement_validateur IS NOT NULL', null, false)
                    ->where('prime_mois', $month)
                    ->where('prime_annee', $year)
                    ->group_by('prime_user, prime_campagne')
                    ->order_by('prime_user ASC')
                    ;
        $query = $this->db->get('t_primeajustement');
        return $query->result();
    }

    public function getQualiteByMonthAndYear(int $month, int $year) : array
    {
        $this->db->select('primequalite_user, primequalite_campagne, primequalite_typecritere, primequalite_mois, primequalite_annee, primequalite_note, primequalite_commentaire')
                    ->join('tr_primetypecritere', 'primequalite_typecritere = primetypecritere_id')
                    ->where('primequalite_mois', $month)
                    ->where('primequalite_annee', $year)
                    ->where('primetypecritere_actif', 1)
                    ->order_by('primequalite_user ASC, primequalite_campagne ASC, primequalite_typecritere ASC')
                    ;
        $query = $this->db->get('t_primequalite');
        return $query->result();
    }

    public function getUserDayWorkedProdMonth(int $user, int $month, int $year) : int
    {
        $this->db->select('COUNT(DISTINCT mcp_date) as dayWorked')
                ->where('MONTH(mcp_date)', $month)
                ->where('YEAR(mcp_date)', $year)
                ->where('mcp_agent', $user)
                ->where('mcp_quantite >', 0)
                ;
        $query = $this->db->get('t_missioncampagneprocess');
        //echo $this->db->last_query();
        if($query->num_rows() == 1) return $query->row()->dayWorked;
        return 0;
    }

    public function savePrimeQualite(array $datasQualite) : int
    {
        $error = 0;
        foreach($datasQualite as $qualite)
        {
            var_dump($qualite);
            $this->db
                    ->where('prime_user', $qualite['prime_user'])
                    ->where('prime_campagne', $qualite['prime_campagne'])
                    ->where('prime_mois', $qualite['prime_mois'])
                    ->where('prime_annee', $qualite['prime_annee'])
            ;
            $this->db->update('t_prime', ['prime_montantqualite' => $qualite['prime_montantqualite'], 'prime_datemodif' => date('Y-m-d H:i:s')]);
            if(!$this->db->affected_rows()) $error++; 
        }
        return $error;
    }

    public function updatePrimeMontantPercu(int $month, int $year)
    {
        $this->db
                ->set('prime_montantpercu', 'prime_basemensuelle + prime_montantqualite + prime_totalbonus - prime_totalmalus', false)
                ->set('prime_datemodif', date('Y-m-d H:i:s'))
                ->where('prime_mois', $month)
                ->where('prime_annee', $year)
                ;
        $this->db->update('t_prime');
        if($this->db->affected_rows()) return true;
        return false; 
    }
}
