<div class="row">
    ...
    <div class="row mt-3">
        <div class="col-md-12">
        <button id="importQualite" class="btn btn-primary mx-2"><i class="fa fa-calendar-plus-o pr-2"></i>Import qualité</button>
        </div>
    </div>
...
<!-- IMPORT QUALITE -->
<div class="modal fade" id="modalimportqualite" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modalimportqualite" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form name="form-import-qualite" method="post" action="<?= site_url('primeproduction/doImportQualite') ?>" enctype="multipart/form-data">>
                <div class="modal-header">
                    <h5 class="modal-title">Import qualite</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3 row">
                        <label for="importqualite_type" class="col-sm-4 col-form-label">Type de qualité </label>
                        <div class="col-sm-8">
                            <select name="importqualite_type" id="primeajustement_type" class="form-control" required>
                                <option value="">Selectionner un type de qualité</option>
                                <?php foreach($listQualite as $qualite): ?>
                                    <option value="<?= $qualite->primetypecritere_id ?>"><?= $qualite->primetypecritere_libelle ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="mcp_client" class="col-sm-4 col-form-label">Mois</label>
                        <div class="col-sm-8">
                            <select name="importqualite_month" id="importqualite_month" class="form-control">
                                <?php
                                    $months = array("Janvier", "Fevrier", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Dec");
                                    foreach ($months as $index => $month) {
                                        echo '<option value="' . ($index+1) . '" '. ( ($index+1) == $filtre['month'] ? 'selected="selected"' : '' ) . '>' . $month . '</option>';
                    ...
                    <div class="mb-3 row">
                        <label for="mcp_client" class="col-sm-4 col-form-label">Année</label>
                        <div class="col-sm-8">
                            <select name="importqualite_year" id="importqualite_year" class="form-control">
                                <?php
                                    $year = date('Y');
                                    $endYear = $year - 5;
                                    for($year; $year > $endYear; $year--){
                    ...
                    <div class="mb-3 row">
                        <label for="mcp_client" class="col-sm-4 col-form-label">Besoin d'un template <i class="fa fa-question-circle"></i></label>
                        <div class="col-sm-8">
                            <button type="button" class="btn btn-sm btn-secondary" id="download_tpl_import_qualite">Telecharger le fichier de template</button>
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="mcp_client" class="col-sm-4 col-form-label">Fichier d'import</label>
                        <div class="col-sm-8">
                            <input type="file" name="importqualite_file" id="importqualite_file" accept=".csv" class="form-control" />
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                    <button type="submit" id="confirmImportQualite" class="btn btn-primary">Confirmer</button>
                </div>
            </form>
    ...
    $(document).ready(function()
    {
        ...
        var validationTable = $("#list-prime").DataTable({
            ...
            columns : [
                ...
                { 
                    data : "prime_montantpercu",
                    render : function(data, type, row){
                        return '<b class="link link-info">' + currencyFormat.format(data) + '</b>'
                    }
                },
                /*{ 
                    data : null,
        ...
        $(document).on('click', '#importQualite', function(){
            $(':input','#modalimportqualite')
                .not(':button, :submit, :reset, :hidden')
                .val('')
                .prop('checked', false)
                .prop('selected', false).trigger( "change" );
            $('#modalimportqualite').modal('show');
        })
        $(document).on("click", "#download_tpl_import_qualite", function () {
            $.fileDownload("<?= base_url('template/tpl_import_qualite.csv') ?>")
            .done(function () {
                alert("Template telecharg�!");
            })
            .fail(function () {
